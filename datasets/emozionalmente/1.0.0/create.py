...
def main():
    ...
    speaker_df = pd.read_csv(user_path)
    ...
    # Add audio dir to file names for correct path
    samples_df['file_name'] = samples_df['file_name'].apply(lambda x: os.path.join(audio_dir_name, x))
    evaluations_df['file_name'] = evaluations_df['file_name'].apply(lambda x: os.path.join(audio_dir_name, x))

    # -------- Add schemes and misc table for speaker--------
    speakers = samples_df['actor'].unique()

    speaker_df = speaker_df[speaker_df['username'].isin(speakers)]
    speaker_df.rename(columns={'username':'speaker'}, inplace=True)
    speaker_df['age'] = speaker_df['age'].astype('int')
    speaker_df['speaker'] = speaker_df['speaker'].astype('string')
    speaker_df.set_index('speaker', inplace=True)
    db.schemes['age'] = audformat.Scheme('int', minimum=0, description='Age of speaker')
    db.schemes['gender'] = audformat.Scheme(labels=['female', 'male', 'other'], description='Gender of speaker')
    db.schemes['mother_tongue'] = audformat.Scheme(labels=['italian'], description='Mother tongue of speaker')
    ...
    db['speaker']['mother_tongue'] = audformat.Column(scheme_id='mother_tongue')
    db['speaker'].set(speaker_df)
    ...
    for split, split_speakers in speaker_info.items():
        ...
        db[f'emotion.categories.{split}.desired']['emotion'] = audformat.Column(
            scheme_id='emotion',
        )
        db[f'emotion.categories.{split}.desired']['emotion'].set(split_df['emotion_expressed'])
